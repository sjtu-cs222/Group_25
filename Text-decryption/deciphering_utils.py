...
def propose_a_move(state,step=3):
    """
    Proposes a new move for the given state,
    step is the number of moves, depending on change of entropy, 
...
#revision
def compute_error(state,ref):
    # print(''.join(state["text"]))
    # print(compute_difference(state["text"],ref),len(ref))
    return float(compute_difference(''.join(state["text"]),ref))/float(len(ref))